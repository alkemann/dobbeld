...
class Debug
{
    ...
    public $options;
    public $output = array('<style type="text/css">@import url("/dobbeld/css/debug.css");</style>');

	/**
	 * Get the singleton instance
	 *
	 * @return dobbeld\util\Debug
	 */
    ...
    public function dump($var, $options = array())
    {
        ...
        switch ($mode) {
			case 'FirePHP':
				$locString = \dobbeld\util\adapters\FirePHP::locationString($location);
				...
				break;
			case 'Json':
				$locString = \dobbeld\util\adapters\Json::locationString($location);
				$this->output[] = array('location' => $locString, 'dump' => $dump);
				break;
			case 'Log' :
				$locString = \dobbeld\util\adapters\Log::locationString($location);
				\lithium\analysis\Logger::debug($locString . "\n" . implode("\n", $dump));
				return;
				break;
			case 'Html' :
			default :
				$locString = \dobbeld\util\adapters\Html::locationString($location);
	...
	public function out($key = null)
    {
		if ($this->options['mode'] == 'Html') {
			\dobbeld\util\adapters\Html::top($this->output, $key);
			return;
		}
		$this->__out($key);
	}
    ...
    {
		$adapter = '\dobbeld\util\adapters\\'. $this->options['mode'];
        if (is_array($var))
